...
Connect-VIServer oaintvcs01 | Out-Null
$error.clear()
$ts = (get-date -uformat %Y%m%d%H%M)
$logpath="\\oaintpwshell\scripts\logs\VMResize-$ts.txt"
function set-processor {
 param($vm,$v_socket,$core)


 Add-Content -Path $logpath -Value "$vm : Changing cores to $core"
 $numcores=$core
 ...
 Add-Content -Path $logpath -Value "$vm : Changing CPUs to $v_socket"
 Get-VM $vm | Set-VM -NumCpu $v_socket -Confirm:$false
}

$csv=Import-Csv \\oaintpwshell\scripts\input\vmresize2.csv

foreach ($record in $csv) {
    $vmserver=$record.servername
    Add-Content -Path $logpath -Value "$vmserver : Shutting down server..."
    Shutdown-VMGuest $vmserver -Verbose -Confirm:$false
    if ($error.Count -gt 0) {
      Write-Output 
      Add-Content -Path $logpath -Value "$vmserver : Error occured during shutdown, trying another command..."
      $error.clear()
      Stop-Computer $vmserver -Verbose -Force
    }
}

foreach ($record in $csv) {
  $vmserver=$record.servername
  $v_socket=$record.NewProcessorCount
  ...
  Do { 
          ...
          $j++        
          Write-Output "$vmserver : Shutting Down...$($j)" 
          If($j -eq 20) { 
            
            Add-Content -Path $logpath -Value "$vmserver : SKIPPING, server did not shutdown for some reason"
            break
            $continue=$false
          }
     } 
          While ((Test-Connection -ComputerName $vmserver -Count 1 -Quiet))    

  if ($continue) {
    set-processor $vmserver $v_socket "1"
    Add-Content -Path $logpath -Value "$vmserver : Restarting server..."
    Start-VM $vmserver -Verbose  
  }
}
Add-Content -Path $logpath -Value "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
Add-Content -Path $logpath -Value "+++++ SEE BELOW FOR ANY ERRORS THAT OCCURRED WHEN THE SCRIPT RAN +++++"
Add-Content -Path $logpath -Value "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
...
$cpuinfo=get-vm -Name ($csv | % { $_.servername }) | Out-String
$body="Please confirm the following servers CPU were adjusted as expected.  Also see attached logfile for any potential problems." + $cpuinfo
Send-MailMessage -From $from -To $to -Subject "VM Resize Job Results" -SmtpServer $smtp -Attachments $logpath -Body $body

Disconnect-VIServer -Force -Confirm:$false
# SIG # Begin signature block
# MIIPWwYJKoZIhvcNAQcCoIIPTDCCD0gCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUqXCS8cawFSDB5nDItSMlcyfY
# oPKgggy/MIIF5zCCBM+gAwIBAgIKYU4uYQAAAAAAAzANBgkqhkiG9w0BAQUFADAm
...
# BAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFDLo4EWddKte
# pk4Dn+ViW5b8v1SIMA0GCSqGSIb3DQEBAQUABIIBABwfaaWyVfLyxrAdsPLF2YVM
# 0lQ3z+vAnDnKWNkYR1u/5DIOXqD6ryDU9gRiz/k//dadxEAAgipsZMjbYuqZ+351
# kiUtRlY6kZnRJuvQxT+ZhrJqqMfiOV5Zso9LTVz0s1kpaJiHHw3fBakjjgwUuFF8
# ltRO4Clyui+Bq6ZHnO2lTG+LRDEGkiSU8f8pqyFc50ak4Bba9URPpDZXFpT+tbLn
# XpdMDz+lznD5g2+keSDop634znOTTLXdE0B2owM8ebZC804WYVAZuU2aefwFQIuy
# 1P8zSHCXvwFDeoxKoiHzVNpBZ9vBQOGV2M2U7o0OejDVGLyiMMpvYBejSTP+aJU=
