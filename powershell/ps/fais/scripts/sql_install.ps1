...
#read-host "Remember to change the servername and SQLS account setup after testing...."
# get local server name
$servername = gc env:computername
###$servername = "oaintegdb2" # for Steve
 
##SQLS account

$sqlsacc= "SSFHS\$servername-SQLS"  # Needs a backslash instead of a forward slash

##$sqlspw= read-host "Enter SQLS password:"
$sqlspw1 = Get-Credential -Credential $sqlsacc
$sqlspw= $sqlspw1.GetNetworkCredential().password


##SQLA account

$sqlaacc= "SSFHS\$servername-SQLA"  # Needs a backslash instead of a forward slash

##$sqlapw= read-host "Enter SQLA password:"
$sqlapw1 = Get-Credential -Credential $sqlaacc
$sqlapw= $sqlapw1.GetNetworkCredential().password
...
if (! $datadrive) {
  $datadrive = read-host "Enter 'S' for single data drive (D) and 'M' for Multiple data drives (F,G,K): "
}

if ($datadrive -eq "S") {
  $install_dir="D:\MSsql"
  $backup_dir="D:\MSsql\Backup\Prod"
  $db_dir="D:\MSsql\Data\Prod"
  $xlog_dir="D:\MSsql\Xlog\Prod"
  $tempdb_dir="D:\MSsql\Tempdb\Data"
  $tempdblog_dir="D:\MSsql\Tempdb\Xlog"
}
if ($datadrive -eq "M") {
  $install_dir="F:\MSsql"
  $backup_dir="K:\MSsql\Backup\Prod"
  $db_dir="F:\MSsql\Data\Prod"
  $xlog_dir="G:\MSsql\Xlog\Prod"
  $tempdb_dir="K:\MSsql\Tempdb\Data"
  $tempdblog_dir="K:\MSsql\Tempdb\Xlog"
}

# Stage SQL install
"Start SQL $sqlversion install..."

#copy config file to server
if ($sqlversion -eq "2008") {
 copy '\\oaintpwshell\c$\input\ConfigurationFile2008.ini' c:\
 $configfile="C:\ConfigurationFile2008.ini"
}
if ($sqlversion -eq "2008R2") {
 copy '\\oaintpwshell\c$\input\ConfigurationFile2008R2.ini' c:\
 $configfile="C:\ConfigurationFile2008R2.ini"
}
if ($sqlversion -eq "2012") {
 copy '\\oaintpwshell\c$\input\ConfigurationFile2012.ini' c:\
 $configfile="C:\ConfigurationFile2012.ini"
}


if ($sqlversion -eq "2008") {
     & '\\oaincofs\DBsource\SQL Server\2008\Standard\Setup.exe' /SQLSVCACCOUNT=$sqlsacc /SQLSVCPASSWORD=$sqlspw /AGTSVCACCOUNT=$sqlaacc /AGTSVCPASSWORD=$sqlapw /SAPWD=$sapw /CONFIGURATIONFILE=$configfile /INSTALLSQLDATADIR=$install_dir /SQLBACKUPDIR=$backup_dir /SQLUSERDBDIR=$db_dir /SQLUSERDBLOGDIR=$xlog_dir /SQLTEMPDBDIR=$tempdb_dir /SQLTEMPDBLOGDIR=$tempdblog_dir

}

if ($sqlversion -eq "2008R2") {
   & '\\oaincofs\DBsource\SQL Server\2008R2\Standard\Setup.exe' /SQLSVCACCOUNT=$sqlsacc /SQLSVCPASSWORD=$sqlspw /AGTSVCACCOUNT=$sqlaacc /AGTSVCPASSWORD=$sqlapw /SAPWD=$sapw /CONFIGURATIONFILE=$configfile /IACCEPTSQLSERVERLICENSETERMS /INSTALLSQLDATADIR=$install_dir /SQLBACKUPDIR=$backup_dir /SQLUSERDBDIR=$db_dir /SQLUSERDBLOGDIR=$xlog_dir /SQLTEMPDBDIR=$tempdb_dir /SQLTEMPDBLOGDIR=$tempdblog_dir

}

if ($sqlversion -eq "2012") {
   & '\\oaincofs\DBsource\SQL Server\2012\Standard\Setup.exe' /SQLSVCACCOUNT=$sqlsacc /SQLSVCPASSWORD=$sqlspw /AGTSVCACCOUNT=$sqlaacc /AGTSVCPASSWORD=$sqlapw /SAPWD=$sapw /CONFIGURATIONFILE=$configfile /IACCEPTSQLSERVERLICENSETERMS /INSTALLSQLDATADIR=$install_dir /SQLBACKUPDIR=$backup_dir /SQLUSERDBDIR=$db_dir /SQLUSERDBLOGDIR=$xlog_dir /SQLTEMPDBDIR=$tempdb_dir /SQLTEMPDBLOGDIR=$tempdblog_dir
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUklsFiN5FGWLJ6tpJNYIdGE6K
# R4Kgggy/MIIF5zCCBM+gAwIBAgIKYU4uYQAAAAAAAzANBgkqhkiG9w0BAQUFADAm
...
# BAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFB/lNuMiDO2p
# FR1hDriNGdceJIEsMA0GCSqGSIb3DQEBAQUABIIBAC/hbnx9ICqwMIJNJffm22F7
# BnVG105nThyxWmRy+DcPLg4Xx5Cf/IRpe4hbthDuyAOP2B3gBct6qVz5onAJKlRM
# hTzU2W8c2ZAhEhqYkQC8IjImnaSLdFvxFcCSw+mKPg59mdkYUuyYizd8i180BeaY
# Ae2+yck8Sg7MIOIcatvSXGuJO3kJMM+j4y+2UPI+DB7X5/3qTSoEQcbOytSKs08B
# IwF4p6S/05KQW5FMVIPn8CYGA1Y+96pQpJ8nk3ziP2AmBfCqCxzYro6Q+bVlAbvu
# nPyuO8H23SKnGieUAyJzNdW8FDRkct4/DVmSxx0oGr8gBl1nwhEMfsRN/HtvD14=
