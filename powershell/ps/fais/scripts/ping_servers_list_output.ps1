...
if (! $showall) { $showall = "Y" }

if (! $filepath) { $filepath = Import-Csv "C:\scripts\data\ADlist.csv" }
  ...
  # Get results of connection test   
   ...
   If ($showall -match "Y") {
     IF($rtn -match 'True') { write-host -ForegroundColor green $_ 
        ...
        $_ | Out-File c:\scripts\data\ADlist2.txt -Append
     }
     ### if not true write server name in red
     ELSE { Write-host -ForegroundColor red $_ 
         $_ | Out-File c:\scripts\data\ADlist-no-ping.txt -Append
     } 
   }
   else {
