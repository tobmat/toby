...
function test_service($service, $displayname, $server, $logfile)
{
 Try {
   ...
   Out-file -append -filepath $logfile -inputobject $output
       ...
       $subject="Powerscribe Script ERROR - $displayname service did not restart after reboot of $server."
       $end_time=get-date
...
## START OF SCRIPT
$logfile="c:\scripts\logs\paceart_restart_services_log.txt"
$start=get-date
...
Try {
 ...
 Stop-Service -force -InputObject (Get-Service -DisplayName "Latitude*" -cn A78NTPACEAR)

 ### Confirm all services have been stopped on OAINTPWRSINF
 if ( (get-service -cn A78NTPACEAR | Where-Object {($_.DisplayName -like "Latitude*" -and $_.Status -eq "Running")} )) 
    {
      w_error "A78NTPACEAR"
    } else {
        ...
        # Start Services
        Start-Service -InputObject (get-service -DisplayName "Latitude*" -ComputerName "A78NTPACEAR")
    }

} Catch {
    "Exiting script and sending an email...."
    Out-file -append -filepath $logfile -inputobject "$($Error[0])"
    $subject="Paceart Script ERROR - service(s) did not shutdown"
    $end_time=get-date
    Out-file -append -filepath $logfile -inputobject "SCRIPT END TIME: $end_time"
    $body=(get-content $logfile | out-string)
    send_mail $subject $body
...
test_service "LATITUDE Integration" "LATITUDE Integration" "A78NTPACEAR" $logfile
test_service "LATITUDE Paceart Integration" "LATITUDE Paceart Integration" "A78NTPACEAR" $logfile

$subject="PaceArt Service Restart Script Completed Successfully"
$end_time=get-date
Out-file -append -filepath $logfile -inputobject "SCRIPT END TIME: $end_time"
$body=(get-content  $logfile | out-string)
send_mail $subject $body
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUNqWl+uae5CdoYj5H3LNamDlr
# ui2gggq0MIIFMTCCBBmgAwIBAgIKQGBHegABAAApbjANBgkqhkiG9w0BAQUFADBL
...
# MQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUNGn5SasoLUZZcygFUbdj
# eTnmKXUwDQYJKoZIhvcNAQEBBQAEgYBMafIH8QZfpbznZY1a0zjv3bm+HtBibQ1R
# hFyckaFJX3d6gXjfgFPuHrI6WYDdb27iXf7NFPqPhAwVFLfjsDA/BM/EDtKsMkMa
# 7GsuQtsQXZu8HOYjkaxkhWnenral4WygV1o1sWtjdvZ8QqYuW337J/sdvMpvzz7z
# lFO4H0DSEQ==
