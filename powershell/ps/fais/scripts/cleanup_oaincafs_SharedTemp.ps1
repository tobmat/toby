# Name:    cleanup_oaincafs_SharedTemp.ps1 
# Author:  Toby Matherly
# Date:    16/May/2013
# Details: Deletes any files or folders older than 7 days from "\\ssfhs\shared\AShared\Groups\SharedTemp" 
#  
# Powershell remoting is enabled on this server and this script is scheduled on oaintpwshell.  This script is run from there.
# Add parameter to pass variables from remote_cleanup_byweekly.ps1 to run this script
#
param($days,$TargetFolder)
$days
$TargetFolder

###$TargetFolder = "\\oaincafs\AShared\Groups\SharedTemp" 
$Now = Get-Date 
$LastWrite = $Now.AddDays(-$days)
$LastWrite2 = $Now.AddDays(-$days)
$Files = get-childitem $TargetFolder -include * | Where {$_.LastWriteTime -le "$LastWrite"}  
   ...
   if ($File.Attributes -eq "Directory")
      {
        ...
        $Files2 = get-childitem $TargetFolder2 -include * -recurse | Where {$_.LastWriteTime -le "$LastWrite2"}  
        foreach ($File2 in $Files2)
        {
         if (! $File2) {
          ...
          if (test-path $File2)
          {
          Remove-Item $File2 -force | out-null # -force is needed to remove read-only files
          }
         }
        } 
      }
      ...
      {
          (get-date).ToString() + " +++ (" + $client + ") Delete this file $File," + $File.LastWriteTime.DayOfYear + "+++" 
          if (test-path $File)
          {
          Remove-Item $File -force | out-null # -force is needed to remove read-only files
          }
      }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUEHZd5G9TD0TgYol8eI5IzfUq
# 9Bigggy/MIIF5zCCBM+gAwIBAgIKYU4uYQAAAAAAAzANBgkqhkiG9w0BAQUFADAm
...
# BAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFJYSLw3TIBLu
# Y10PlpozcEIlTcu1MA0GCSqGSIb3DQEBAQUABIIBAFSUrmTCkEEeWfad0KrWFXHK
# APkyF6rCYvqkowTJakBd7Z8bOvr9P5cZ/M3InVMpC2gjNrX8Ka4prAbtL3uWxvDx
# 3pghyd31pgyj8TXTSH7ro6YvZxUtGpEOdLGEsbKMx2OBCVQTf8dVlryePF2rfoU1
# jzzsTcbculZ4zhg3odgOUXdfWbuWqk+uDwhQgm+SkFyQcRZ/5IysLCpiskvCQTmK
# wdTy7XvNzNxPDJ3/MNiA0IsJ3wgT/R+2VSs1gpTV1FqFyp/sXe6S2/kclrzOxaQJ
# zF77B5P7Zf9DjiMyL9wyEQmXEVUsc2I2y2RJh5yFiJehPOXHPpRtBXh6xYN8mf4=
