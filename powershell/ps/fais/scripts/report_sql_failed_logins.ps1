...
#Get-Content C:\scripts\data\sql_servers2.txt | % {

#   $test=Invoke-Sqlcmd -ServerInstance $_ -Query "EXEC sp_readerrorlog 0, 1, 'login failed'"

#   if ( $test.Count -gt 500 ) {
#     $_  + " : " +  $test.Count
#     $test.text | sort -Unique
#     " "
#   }
#} 
 Import-Module sqlps
 $log="C:\scripts\logs\report_sql_failed_logins.txt" 
 "Count of Failed logins : " + (Get-Date).ToString() | Out-File $log

 $s=Invoke-Sqlcmd -ServerInstance oaintsqlmsx\sql2k8 -InputFile \\oaintpwshell\scripts\sql_cms.sql
 $s | % { 
   #"+++" + $_.servername + "+++"
   if ($_.groupname.tostring() -like "SQL 2000*"){
      $_.servername | % {
        
        ###$test3=$test2.errorlog  ### Only works in powershell 3
        $test3 = ($test2 | % {$_.errorlog })
        $test4=($test3 | % { $_.tostring() | Select-String "Login" })
        if ( $test4.count -gt 500 ) {
          $_  + " : " +  $test4.Count + " Windows 2000"  | out-file -Append $log
          $test4 | % { $_.tostring().substring(23,40) } | sort -Unique  | out-file -Append $log
          " "  | out-file -Append $log
        } 
      }
   } else {
      $_.servername | % {
       ...
       if ( $test.Count -gt 500 ) {
         $_  + " : " +  $test.Count  | out-file -Append $log
         ###$test.text | sort -Unique ### Only works in powershell 3
         $test | % { $_.text } | sort -Unique  | out-file -Append $log
         " "  | out-file -Append $log
       }
   }
  }
...
$subject = "SQL Login Errors"
$body = (get-content $log | out-string)
$to = "toby.matherly@franciscanalliance.org"
\\oaintpwshell\scripts\send_mail.ps1 $subject $body $to

# SIG # Begin signature block
# MIIPWwYJKoZIhvcNAQcCoIIPTDCCD0gCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUMaVDCKSR6iBnwEd9WYkd0GEA
# 2Jygggy/MIIF5zCCBM+gAwIBAgIKYU4uYQAAAAAAAzANBgkqhkiG9w0BAQUFADAm
...
# BAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFPU9kj70WM60
# NpQvKEmNV+OzV/nlMA0GCSqGSIb3DQEBAQUABIIBAAVIDC1DJSPWpC0t7qVhiYQx
# aC8GMbLAjUcVTjQlh18IUaCx7Ayvp7xd9WkA7M/8i3j/7TDSiDqnwygYMIt04Ad8
# glKaQMQ5PEJ0bskZokCHhICL7kfIr/lV0DRVG4xr930IPqxYjOdreFMNZMon8wNR
# 9bzTvw8yPh4Qr5/8YsqjNHRb2FxbidsxidwGQbokgyCMqJr1SglOuATY8TA0F76z
# HT+hmSOZ6fm1rYOnmB0NlCfVb47IxNs8mELpGXbdh69Wv8pCG9b9nbWcC2gtN56U
# v3qlfXmBmoYyd2HqcTk6quD46dLdRHZ8HPgoc4ZYBv0TRGtIC+DfpkxfoQSlw24=
